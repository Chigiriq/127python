...
# Place the missing methods here.  Do not modify the code below or above.
    ...
    def read_series_information(self, info_file):
        ...
        while input_line:
            ...
            self.number = info_list[0]               # number
            self.name = info_list[1]                      # name
            self.value = info_list[2]        # value

            # print(WackyPackageCard(int(self.number), self.name, float(self.value)))
    ...
    def __str__(self):
        ...
        # return "{:<10d}{:25}{:10.2f}{:10d}".format(int(self.number), self.name, float(self.value), 0)          
        return "\n"                                 
